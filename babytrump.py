...
class TwitterBabyMonitor():
  ...
  def was_in_last_minute(self, created_at_in_seconds):
    one_minute_ago = datetime.datetime.now() - datetime.timedelta(minutes=1)
    return created_at_in_seconds >= one_minute_ago.timestamp()

  def read_stream(self, only_last_minute=False):
    ...
    if only_last_minute:
      statuses = [s for s in statuses if self.was_in_last_minute(s.created_at_in_seconds)]
